...
export const CACHE_TTL_PRESET_OPTIONS: Array<{
  value: Exclude<AIOMetadataCacheTtlPreset, 'custom'>;
  label: string;
  seconds: number;
}> = [
...
const CACHE_TTL_SECONDS_BY_PRESET = new Map(
  CACHE_TTL_PRESET_OPTIONS.map((option) => [option.value, option.seconds] as const)
);

const CACHE_TTL_PRESET_BY_SECONDS = new Map(
  CACHE_TTL_PRESET_OPTIONS.map((option) => [option.seconds, option.value] as const)
);

export function cacheTtlSecondsFromPreset(
  preset: Exclude<AIOMetadataCacheTtlPreset, 'custom'> | 'custom'
) {
  if (preset === 'custom') return undefined;
  return CACHE_TTL_SECONDS_BY_PRESET.get(preset);
}

export function detectCacheTtlPreset(
  seconds: number | undefined
): AIOMetadataCacheTtlPreset | undefined {
  if (typeof seconds !== 'number') return undefined;
  return CACHE_TTL_PRESET_BY_SECONDS.get(seconds) ?? 'custom';
}

export function formatCacheTtlLabel(seconds: number | undefined) {
  if (typeof seconds !== 'number') return 'Select cache TTL';
  if (seconds % 3600 === 0) {
