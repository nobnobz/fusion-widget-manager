...
export interface UsedAnilistCatalogReference {
  widgetId: string;
  widgetTitle: string;
  widgetIndex: number;
  itemId?: string;
...
function isAnilistCatalogId(catalogId: string): boolean {
  return getCatalogActualId(catalogId).startsWith('anilist.');
}
...
function dedupeReferences(references: UsedAnilistCatalogReference[]): UsedAnilistCatalogReference[] {
  const seen = new Set<string>();
  const deduped: UsedAnilistCatalogReference[] = [];
...
export function collectUsedAnilistCatalogs(
  config: FusionWidgetsConfig,
  manifestCatalogs: AIOMetadataCatalog[] = []
): UsedAnilistCatalogReference[] {
  const references: UsedAnilistCatalogReference[] = [];
  ...
  config.widgets.forEach((widget, widgetIndex) => {
    ...
    if (widget.type === 'row.classic') {
      if (!isAIOMetadataDataSource(widget.dataSource) || !isAnilistCatalogId(widget.dataSource.payload.catalogId)) {
        return;
      }
    ...
    widget.dataSource.payload.items.forEach((item, itemIndex) => {
      ...
      item.dataSources.forEach((dataSource, dataSourceIndex) => {
        if (!isAIOMetadataDataSource(dataSource) || !isAnilistCatalogId(dataSource.payload.catalogId)) {
          return;
        }
...
export function hasUsedAnilistCatalogs(config: FusionWidgetsConfig): boolean {
  return config.widgets.some((widget) => {
    if (widget.type === 'row.classic') {
      return isAIOMetadataDataSource(widget.dataSource) && isAnilistCatalogId(widget.dataSource.payload.catalogId);
    }

    return widget.dataSource.payload.items.some((item) =>
      item.dataSources.some(
        (dataSource) => isAIOMetadataDataSource(dataSource) && isAnilistCatalogId(dataSource.payload.catalogId)
      )
    );
  });
}

export function buildAiometadataAnilistCatalogsOnlyExport(
  config: FusionWidgetsConfig,
  manifestCatalogs: AIOMetadataCatalog[] = [],
  ...
  const catalogs = dedupeReferences(
    collectUsedAnilistCatalogs(config, manifestCatalogs)
