...
export interface UsedAiometadataTraktCatalogReference {
  widgetId: string;
  widgetTitle: string;
  widgetIndex: number;
  itemId?: string;
...
function isAiometadataTraktCatalogId(catalogId: string): boolean {
  return getCatalogActualId(catalogId).startsWith('trakt.');
}
...
function dedupeReferences(
  references: UsedAiometadataTraktCatalogReference[]
): UsedAiometadataTraktCatalogReference[] {
  const seen = new Set<string>();
  const deduped: UsedAiometadataTraktCatalogReference[] = [];
...
export function collectUsedAiometadataTraktCatalogs(
  config: FusionWidgetsConfig,
  manifestCatalogs: AIOMetadataCatalog[] = []
): UsedAiometadataTraktCatalogReference[] {
  const references: UsedAiometadataTraktCatalogReference[] = [];
  ...
  config.widgets.forEach((widget, widgetIndex) => {
    ...
    if (widget.type === 'row.classic') {
      if (!isAIOMetadataDataSource(widget.dataSource) || !isAiometadataTraktCatalogId(widget.dataSource.payload.catalogId)) {
        return;
      }
    ...
    widget.dataSource.payload.items.forEach((item, itemIndex) => {
      ...
      item.dataSources.forEach((dataSource, dataSourceIndex) => {
        if (!isAIOMetadataDataSource(dataSource) || !isAiometadataTraktCatalogId(dataSource.payload.catalogId)) {
          return;
        }
...
export function hasUsedAiometadataTraktCatalogs(config: FusionWidgetsConfig): boolean {
  return config.widgets.some((widget) => {
    if (widget.type === 'row.classic') {
      return isAIOMetadataDataSource(widget.dataSource) && isAiometadataTraktCatalogId(widget.dataSource.payload.catalogId);
    }

    return widget.dataSource.payload.items.some((item) =>
      item.dataSources.some(
        (dataSource) => isAIOMetadataDataSource(dataSource) && isAiometadataTraktCatalogId(dataSource.payload.catalogId)
      )
    );
  });
}

export function buildAiometadataTraktCatalogsOnlyExport(
  config: FusionWidgetsConfig,
  manifestCatalogs: AIOMetadataCatalog[] = [],
  ...
  const catalogs = dedupeReferences(
    collectUsedAiometadataTraktCatalogs(config, manifestCatalogs)
